...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (user.password !== password) {
        ...
        const userWithoutExcludedFields: IUser = user.select(userFieldsToExclude); // Exclude the specified fields

        // ...

        return res.status(200).json({ message: 'User logged in successfully', data: userWithoutExcludedFields });
    }
